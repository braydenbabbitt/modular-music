...
export const OutputSelectorModal = ({ open, moduleId, onSave, onClose, initOutput }: OutputSelectorModalProps) => {
  const mantineTheme = useMantineTheme();
  const supabaseClient = useSupabase();
  const spotifyToken = useSpotifyToken();
  const userPlaylistsQuery = useQuery('user-playlists', () => getUserPlaylists(spotifyToken!), {
    enabled: !!spotifyToken,
    onSuccess: (data) => {
      setShowLoader(false);
      form.setFieldValue('output', stringifyObject(data.find((playlist) => playlist.id === initOutput?.playlist_id)));
    },
  });
  ...
  const form = useForm<OutputFormTypes>({
    initialValues: {
      output: stringifyObject(userPlaylistsQuery.data?.find((playlist) => playlist.id === initOutput?.playlist_id)),
          ...
          <Stack>
            ...
            <Group spacing='xs' align='center'>
              ...
              <InfoHoverButton size={20}>
                <Stack spacing='xs'>
                  <Text size='sm'>
                    <span css={{ fontWeight: 'bold' }}>Overwrite:</span> Replace all songs in the playlist with the
                    songs generated by the module.
                  </Text>
                  <Text size='sm'>
                    <span css={{ fontWeight: 'bold' }}>Append:</span> Add the songs generated by the module to the end
                    of the playlist.
                  </Text>
                  <Text size='sm'>
              ...
              <Stack my='lg' spacing='sm'>
                ...
                <SelectCreatePlaylist
                  onCreate={(playlistId: string) => {
                    userPlaylistsQuery.refetch().then((result) => {
                      if (result.data) {
                        form.setFieldValue(
                          'output',
                          stringifyObject(result.data.find((item) => item.id === playlistId)),
                        );
                      }
                    });
                  }}
